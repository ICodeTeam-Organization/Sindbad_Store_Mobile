...
class GetCategoryNamesCubit extends Cubit<GetCategoryNamesState> {
  ...
  List<CategoryEntity> allCategories = []; // to store all categories
  ...
  Future<void> getMainAndSubCategory({
    ...
    result.fold(
      ...
      for (int i = 0; i < mainAndSubCategory.length; i++) {
        ...
        allCategories.add(mainAndSubCategory[i]);
      }
  ...
  void updateSubCategories(int selectedMainCategoryId) {
    ...
    print("Subcategories count: ${subCategories.length}");
    emit(UpdateSubCategoryState(seleted: true, subCategories: subCategories));
  }
}
